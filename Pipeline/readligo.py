...
class FileList():
    ...
    def findfile(self, gps, ifo):
        # Teja: Fix to be an integer
        start_gps = int(gps - (gps % 4096))
...
def getstrain(start, stop, ifo, filelist=None, strain_chan=None, dq_chan=None, inj_chan=None):
    ...
    if (sl[0][0] == start) and (sl[0][1] == stop):
        ...
        # Teja: Downgrade to warning, since we can inpaint missing segments
        ...
        # {0}""".format(segList))
        print("""Warning: Requested times include times where the data file 
        was not found or instrument not in SCIENCE mode.
        The science mode segment list for the requested time range is: 
        {0}""".format(segList))
    ...
    for time in gpsList:
        ...
        # Teja: We downgraded the error, so capture case of missing file
        if filename is None:
            raise RuntimeError(
                """Given interval encompasses an entire missing file, 
                split the analysis here""")
    ...
    # meta['start'] = start
    # meta['stop']  = stop
    # TEJA: Fixing to return times at actual indices
    meta['start'] = m_start + lndx * dt
    meta['stop'] = m_start + rndx * dt
    meta['dt']    = dt
